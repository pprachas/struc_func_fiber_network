...
init_c0 = [1e-4,1e-5,1e-6,1e-7,1e-8,1e-9,1e-10][int(sys.argv[1])-1]
max_damping = 1e-4#[1e-2, 5e-3,1e-3, 5e-4, 1e-4, 5e-5, 1e-5][int(sys.argv[1])-1]
num_segments = 20


# Name of mesh
mesh_name= f'sensitivity/mesh/n{n}/voronoi{seed}.xdmf'
f_name = f'plots/sensitivity/c0/c0{init_c0}'
...
fea.run_fea(mesh_name, f_name, r, L, step_size, ['r','r'], 1.5*crit_strain, compute_individual=False, max_damping = max_damping, residual = True, init_c0 = init_c0, abs_tol = 1e-10, rel_tol = 1e-12) # make runs stop after strain stiffening 1.3*0.12541075747605646