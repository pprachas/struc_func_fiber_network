...
init_c0 =  1e-7#1e-7 #[1e-4,1e-5,1e-6,1e-7,1e-8,1e-9,1e-10][int(sys.argv[1])-1]
max_res = [5e-3,1e-3, 5e-4, 1e-4, 5e-5,1e-5][int(sys.argv[1])-1] #[1e-2, 5e-3,1e-3, 5e-4, 1e-4, 5e-5,1e-5, 5e-6,1e-6][int(sys.argv[1])-1] #[1e-2, 5e-3,1e-3, 5e-4, 1e-4, 5e-5][int(sys.argv[1])-1]
num_segments = 15
...
crit_strain, diff_energy = fea.run_critical_strain(mesh_name, r, L, step_size, ['r','r'], abs_tol = 1e-8, rel_tol = 1e-10, init_c0 = init_c0, max_damping_res = max_res)

np.savetxt(f'{f_name}/crit_strain.txt', np.array([crit_strain]))

print('Starting FEA run!')

# crit_strain = 1171.4583333331393/L
